...
AS $$
BEGIN
  INSERT INTO public.profiles (user_id, email, referral_code, last_login)
  VALUES (NEW.id, NEW.email, generate_referral_code(), now());
  RETURN NEW;
END;
$$;
...
CREATE POLICY "Users can view their own saved briefs"
  ON public.saved_briefs FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own saved briefs"
  ON public.saved_briefs FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own saved briefs"
  ON public.saved_briefs FOR DELETE USING (auth.uid() = user_id);
