...
export default function SocialProof() {
  return (
    <section className="py-20 relative" aria-labelledby="proof-heading">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <p className="text-xs font-mono text-primary tracking-widest mb-3">WHAT SECURITY LEADERS SAY</p>
          <h2 id="proof-heading" className="text-3xl md:text-4xl font-bold text-foreground">
            Trusted by teams that <span className="gradient-text">can't afford blind spots.</span>
          </h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto" role="list" aria-label="Testimonials">
          {testimonials.map((t) => (
            <blockquote
              key={t.role}
              ...
              role="listitem"
            >
              <Quote className="h-5 w-5 text-warning/60" aria-hidden="true" />
              <p className="text-foreground font-medium leading-relaxed">"{t.quote}"</p>
              <footer className="text-xs font-mono text-muted-foreground tracking-wide">— {t.role}</footer>
            </blockquote>
          ))}
        </div>
      </div>
